...
class EncryptionLayer {
   ...
   */
  initELayer(password, pk = null, sk = null) {
    return protectSK(password)
        .then((protectKey) => {
          if (pk && sk) {
            return importPK(pk)
...
function importSK(SK, protectKey, randVal){
  let randomValues = Buffer.from(randVal, "base64")

  return crypto.subtle.unwrapKey(
      "jwk",
      JSON.parse(Buffer.from(SK, "base64")),
      protectKey,
      {   //these are the wrapping key's algorithm options
        name: "AES-GCM",
        iv: randomValues,
        additionalData: randomValues.slice(0,8).reverse(),
        tagLength: tagLength
      },
...
function exportSK(SK, protectKey) {
  ...
  return window.crypto.subtle.exportKey("jwk", SK)
      ...
      // TODO: convert from 2 objects to single base64 string
      .then((keydata) => ({
        privateKey : Buffer.from(JSON.stringify(keydata)).toString("base64"),
        randomValues : Buffer.from(randomValues).toString("base64")
      }))
