...
class EncryptionLayer {
  ...
  /** Initialize the encryption layer. If public and private key are provided (pk and sk)
   ...
   * @param pk            {string}    the base64 encoded public key
   * @param sk            {string}    the base64 encoded private key
  ...
  initEncLayer(password, pk = null, sk = null, info = {}, storeLocally = false) {
    return getSKWrapper(password, info, storeLocally)
        ...
        .catch((err) => {
          throw new Error(`Impossible to initialize encryptionlayer:\n${err}`);
        });
  }

  /** Wrap key given key and return the result as a base64 encoded string
   *
   * @param key     {CryptoKey} The CryptoKey to be wrapped
   ...
   * @param update      {boolean} Select if is necessary to unwrap again the key,
   *                              even if it has been already unwrapped before. Default is false
   * @return {Promise.<undefined>}
   */
  static unwrapKey(wrappedKey, sbox, update = false) {
    ...
    if (!ck || update) {
      return crypto.subtle.unwrapKey(
        ...
        .then((key) => { commonKeys.set(sbox, key) })
  ...
  static generateCommonKey(sbox) {
    return crypto.subtle.generateKey(
        ...
        .then((key) => { commonKeys.set(sbox, key); return key })
