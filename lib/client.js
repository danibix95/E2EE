...
class User {
  ...
  updateAuth() {
    ...
    return API.post(`/auth/token`, data, _(this).auth, API.CONT_TYPES.FORM_DATA)
        .then((result) => {
          ...
          _(this).rfst = result.data.refresh_token;
  ...
  static login(username, chinoPassword, keyPassword, info = {}) {
    if (!username || !chinoPassword || !keyPassword) throw new Error("Missing credentials");
    ...
    return API.post(`/auth/token`, loginData, null, API.CONT_TYPES.FORM_DATA)
        ...
        .then((result) => {
          if (result.data.documents.length !== 1) throw new Error("There's something wrong with this user!");
        ...
        .catch((error) => { throw new Error(`Impossible to login with this credentials:\n${error}`)});
  }
