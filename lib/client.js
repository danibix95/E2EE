...
class User {
  ...
  deleteAccount(adminAuth) {
    ...
    return listSearchDocs(linkSchema, filter, _(this).auth, documents)
        ...
        .then(() => API.del(`/users/${this.userId}`, {force: true}, adminAuth))
  ...
  grantAccess(caller, username) {
    ...
    return caller.search(username)  // get User id
        ...
        .then(() => { // get the selected user public key
          ...
          return API.post(`/search/${keysSchema}`, filter, _(caller).auth)
              .then((res) => {
                if (res.data.documents.length < 1) throw new Error("No Public Key found");
                res.data.documents[0].content.public_key
              });
        })
        .then((PK) => {
    ...
    const chunkAndUpload = () => {
      ...
      return API.post(`/schemas/${_(this).filesId}/documents`, upDoc, _(caller).auth)
          ...
          .then((result) => {
            ...
            return API.put(`/documents/${result.data.blob.document_id}`, upDoc, {}, _(caller).auth)
          })
          .then(() => true)
  ...
  retrieveFiles(caller, lastDownload = null, toSort = true) {
    ...
    const downloadFiles = () =>
        selectRetriever()
            ...
            .then((docs) => {"STOP", console.log(docs[0]); throw new Error("Stop")})
            .then((docs) =>
              Promise.all(docs.map((d) => {
                console.log(d);
                return API.getBlob(`/blobs/${d.content.enc_file}`, {}, _(caller).auth)
                      .then((cipher) => {
                        console.log(cipher);
                        return ({
