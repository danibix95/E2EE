...
class User {
  ...
  static search(username, auth) {
    ...
    return API.post(`/search/users/${userSchema}`, filter, auth)
        .then((result) => result.data.count === 1 ? result.data.users[0].user_id : null);
  }

  static deleteAccount(user, auth) {
...
class SBox {
  ...
  grantAccess(caller, username) {
    ...
    return User.search(username, _(caller).auth)  // get User id
        .then((usrId) => {
  ...
  syncUsers(caller) {
    ...
    return listSearchDocs(linkSchema, filter, _(caller).auth, documents)
        ...
        .catch((error) => new Error(`Impossible to synchronize user of this SBox:\n${error}`));
  }
   ...
   * @return {Promise.<boolean>}
   */
  remove(doc_id, caller) {
    return API.del(`/documents/${doc_id}`, {force:true}, _(caller).auth)
        .then(() => true)
   ...
   * @return {Promise.<boolean>}
   */
  removeFile(file_id, caller) {
    return API.get(`/documents/${file_id}`,{}, _(caller).auth)
        .then((result) => API.del(`/blob/${result.data.content.blob_id}`, {}, _(caller).auth))
        .then(() => API.del(`/documents/${file_id}`, {force:true}, _(caller).auth))
        .then(() => true)
        .catch((error) => {throw new Error(`Impossible to delete selected file:\n${error}`)})
  }
}
